...
This script mirrors the functionality of ``downloader.sh`` but uses Python and
only writes to ``/tmp`` while the final output is copied to the destination
(UC path). A marker directory is created for each timestamped download so that
subsequent executions skip previously processed files.
"""
...
def main() -> None:
    ...
    parser.add_argument('--root', default='/Volumes/edsm/bronze/landing/data', help='Destination root path (UC path)')
    parser.add_argument('--marker-root', default='/Volumes/edsm/bronze/landing/markers', help='Marker directory root')
